...
构建一个完美的 ZFS 存储环境分为两步：**第一步创建作为“属性模板”的根存储池，第二步创建实际存放数据的子数据集。**
...
在几乎所有的 ZFS 官方文档和教程中，存储池的名字总是叫 `tank`。这并非巧合，而是包含了开源先驱们的物理隐喻与极客浪漫：
...
这套配置之所以强大，是因为每一行参数都解决了一个特定的性能瓶颈。

#### 1. 绝对的底层基石（建池规则）

* **永远使用硬盘 ID (`/dev/disk/by-id/...`)**
  * **原理**：Linux 重启时，`/dev/sda`、`/dev/sdb` 这样的盘符可能会发生漂移变化。使用全球唯一的硬盘物理 ID 建池，可以保证无论硬盘插在哪个接口，ZFS 都能准确无误地识别并导入阵列。
* **`-o ashift=12`（最致命的属性，建池后不可更改！）**
  * **原理**：`ashift` 决定了底层最小写入扇区大小（$2^{12} = 4096$ 字节）。现代硬盘即使物理扇区是 4K，为了兼容老系统，也会向操作系统“谎称”自己是 512 字节（512e）。如果不强制设为 `12`，ZFS 会信以为真，导致每次写入都需要进行极度消耗性能的“读-改-写”操作。强制 `ashift=12` 可彻底消除这种写放大，保护性能和硬盘寿命。注意这里是小写的 `-o`，代表 Pool 级别的底层属性。

#### 2. 空间与性能优化组合拳（数据集级别 `-O`）
